...
#include "UCT.hpp"
#include<math.h>
Node **Node::NodeList = new Node *[MAX_SIZE];
size_t Node::cnt = 0;

Node::Node(int M,int N,int noX,int noY,int _x,int _y,bool turn,Node*par):
    M(M),N(N),noX(noX),noY(noY),_x(_x),_y(_y),turn(turn),parent(par){
    optionNum = 0;
    options = new int[N];
    child = new Node *[N];
    for (int i = 0; i < N;i++){
...
bool Node::end(){
    ...
    if(_x==-1&&_y==-1)
        return flag = 0;
    if(!turn&&machineWin(_x,_y,M,N,UCT::board))
        return flag = 1;
    if(turn &&userWin(_x,_y,M,N,UCT::board))
        return flag = 1;
    if(isTie(N,UCT::top))
        return flag = 1;
    return flag = 0;
...
Node *Node::expand(){
    ...
    while(1){
        ...
        if(noX==newX-1&&noY==newY)
            --UCT::top[newY];
        if(!UCT::top[newY]||!optionNum)break;//这一列到顶端或节点全扩展完
...
Node *Node::newNode(int M,int N,int noX,int noY,int _x,int _y,bool turn,Node*par){
    ...
    if(!NodeList[cnt]){
        return NodeList[cnt++] = new Node(M, N, noX, noY, _x, _y, turn, par);
    }
    NodeList[cnt]->set(M, N, noX, noY, _x, _y, turn, par);
    return NodeList[cnt++];
}
